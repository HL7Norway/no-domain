...
* ^url = "http://hl7.org/fhir/StructureDefinition/NoDomainVitalSignsObservationCuffSize"
* ^version = "0.9.0000"
* ^status = #draft
* ^date = "2021-05-01"
* ^context.type = #element
* ^context.expression = "Observation"
* url = "http://hl7.org/fhir/StructureDefinition/NoDomainVitalSignsObservationCuffSize" (exactly)
* value[x] only CodeableConcept
* value[x] from NoDomainVitalSignsObservationCuffsize (required)
* value[x].coding ..1
* value[x].coding from NoDomainVitalSignsObservationCuffsize (required)
